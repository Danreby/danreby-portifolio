  ...
  const socials = [
    {
      name: 'GitHub',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="w-12 h-12 transform transition-transform duration-200 group-hover:scale-130 group-hover:text-blue-200" viewBox="0 0 16 16" fill="currentColor">
      ...
      <RevealOnScroll>
        <div className="relative p-8 rounded-lg findme-led-border mx-auto w-11/12 max-w-4xl">
          ...
          <ul className="flex flex-wrap justify-center gap-8">
            {socials.map((social) => (
              <li key={social.name} className="group flex flex-col items-center">
                <a
                  href={social.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={social.name}
                  className="text-blue-600/30 hover:text-blue-200 transition-colors duration-200"
                >
                  {social.icon}
                </a>
                <span className="mt-2 text-sm text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  {social.name}
                </span>
              </li>
            ))}
